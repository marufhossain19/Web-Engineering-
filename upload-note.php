    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        if (!in_array($filetype, $allowed)) {
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $destination)) {
                ...
                $stmt = $pdo->prepare("INSERT INTO notes (user_id, title, course_code, teacher, semester, year, file_path, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$_SESSION['user_id'], $title, $course_code, $teacher, $semester, $year, $destination, $description]);
        ...
        <div class="bg-gray-800 rounded-2xl border border-gray-700 p-8">
            ...
            <?php if ($success): ?>
                <div class="bg-green-500/10 border border-green-500 text-green-500 px-4 py-3 rounded-lg mb-6">
                    <?= htmlspecialchars($success) ?> <a href="notes.php" class="underline font-semibold">View all notes</a>
                </div>
            <?php endif; ?>
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">Course Teacher *</label>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              class="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                              placeholder="Add a brief description of the note content..."></textarea>
                    <p class="text-xs text-gray-400 mt-2">This will help others understand what's in the note</p>
                </div>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="notes.php" class="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-3 rounded-lg font-semibold transition text-center">
                        Cancel
                    </a>
                </div>
            </form>
